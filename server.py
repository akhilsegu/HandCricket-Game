...
def threaded_client(conne, p, gameId):
    global idCount
    conne.send(str.encode(str(p)))

    reply = ""
    while True:
        try:
            data = conne.recv(4096).decode()
            ...
            if gameId in games:
                ...
                if not data:
                    ...
                    conne.sendall(pickle.dumps(game))
            else:
                break
        except:
            break
...
while True:
    ...
    start_new_thread(threaded_client, (conne, p, gameId))
